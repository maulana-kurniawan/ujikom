    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <a href="{{ route('tambah-anggota') }}" class="btn btn-teal" aria-label="Tambah Data">
                        Tambah Data
                    </a>
                </div>
            </div>
                    ...
                    <table id="tabel" class="table card-table table-vcenter text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">{{ date('d F Y', strtotime($anggota->tanggal_lahir)) }}</td>
                                <td>{{ $anggota->alamat }}</td>
                                <td class="text-center">{{ $anggota->no_hp }}</td>
                                <td class="text-center">{{ $anggota->jenis_id }}</td>
                                <td class="text-center">{{ $anggota->jenis_anggota }}</td>
                                <td class="text-center">{{ $anggota->status }}</td>
                                <td class="text-center">{{ $anggota->jumlah_pinjam }}</td>
                                <td class="text-center">
                                    <form method="post" action="{{ route('anggota.destroy', $anggota->kode_anggota) }}">
                                        @csrf
                                        <a href="{{ route('anggota.edit', $anggota->kode_anggota) }}" class="btn btn-teal btn-icon" title="Ubah data anggota ini" data-bs-toggle="tooltip" data-bs-placement="bottom">
                                        ...
                                        <button type="submit" class="btn btn-red btn-icon" title="Hapus data anggota ini" data-bs-toggle="tooltip" data-bs-placement="bottom">
