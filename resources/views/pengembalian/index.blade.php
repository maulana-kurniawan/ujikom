    ...
    <div class="container-xl">
        <div class="row g-2 align-items-center">
            ...
            <div class="col-auto ms-auto d-print-none">
                <div class="btn-list">
                    <a href="{{ route('tambah-pengembalian') }}" class="btn btn-teal" aria-label="Tambah Data">
                        Tambah Data
                    </a>
                </div>
            </div>
                    ...
                    <table id="tabel" class="table card-table table-vcenter text-nowrap">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">{{ date('d F Y', strtotime($pengembalian->tanggal_pinjam)) }}</td>
                                <td class="text-center">{{ date('d F Y', strtotime($pengembalian->tanggal_kembali)) }}</td>
                                <td>{{ $pengembalian->judul }}</td>
                                <td>{{ $pengembalian->jenis_bahan_pustaka }}</td>
                                <td>{{ $pengembalian->jenis_koleksi }}</td>
                                <td>{{ $pengembalian->jenis_media }}</td>
                                <td>Rp<span class="float-end">{{ number_format($pengembalian->denda, 0, ".", ".") }}</span></td>
                                <td>{{ $pengembalian->keterangan }}</td>
                                <td class="text-center">
                                    <form method="post" action="{{ route('pengembalian.destroy', $pengembalian->no_transaksi_kembali) }}">
                                        @csrf
                                        <a href="{{ route('pengembalian.edit', $pengembalian->no_transaksi_kembali) }}" class="btn btn-teal btn-icon" title="Ubah data pengembalian ini" data-bs-toggle="tooltip" data-bs-placement="bottom">
                                        ...
                                        <button type="submit" class="btn btn-red btn-icon" title="Hapus data pengembalian ini" data-bs-toggle="tooltip" data-bs-placement="bottom">
